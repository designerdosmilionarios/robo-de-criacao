...
// === UI Cards (Mockups SVG inline para composicao premium) ===

const UICardContainer: React.FC<{ children: React.ReactNode; className?: string; style?: React.CSSProperties }> = ({ children, className = '', style }) => (
  <div
    ...
    style={{
      ...
      boxShadow: '0 30px 80px rgba(0,0,0,0.55), 0 0 0 1px rgba(255,255,255,0.05) inset, 0 -20px 40px rgba(0,0,0,0.3) inset',
      ...style,
    }}
  >
    {children}
...
const PhoneCard: React.FC<{ variant?: string }> = ({ variant = 'analytics' }) => (
  <UICardContainer className="h-full">
    {/* Frame do celular com notch e status bar */}
    <div className="p-1.5 h-full flex flex-col">
      ...
      >
        {/* Status bar */}
        <div className="absolute top-0 left-0 right-0 flex justify-between items-center px-3 pt-1.5 z-10 text-[8px] text-white/80 font-bold">
          <span>9:41</span>
        ...
        {/* Notch */}
        <div className="absolute top-1.5 left-1/2 -translate-x-1/2 w-12 h-3 rounded-full bg-black z-20" />

        {variant === 'analytics' && (
          <div className="p-2 pt-6 text-white space-y-1.5">
            <div className="text-[8px] font-bold text-emerald-400 uppercase">Audience</div>
            <div className="grid grid-cols-3 gap-1.5 text-center">
              <div className="bg-white/10 rounded p-1">
                <div className="text-[10px] font-black">125K</div>
                <div className="text-[6px] text-white/60">Seguidores</div>
              </div>
              <div className="bg-white/10 rounded p-1">
                <div className="text-[10px] font-black">18K</div>
                <div className="text-[6px] text-white/60">Likes</div>
              </div>
              <div className="bg-white/10 rounded p-1">
                <div className="text-[10px] font-black text-red-400">2</div>
                <div className="text-[6px] text-white/60">Agenda</div>
              </div>
            </div>
            <div className="bg-white/5 rounded h-10 flex items-end gap-0.5 p-1">
        ...
        {variant === 'chat' && (
          <div className="p-2 pt-6 text-white space-y-1 text-[8px]">
            <div className="flex items-center gap-1 pb-1 border-b border-white/10">
              <div className="w-4 h-4 rounded-full bg-gradient-to-br from-emerald-400 to-cyan-500" />
              <div>
                <div className="font-bold">Dr. Apoio</div>
                <div className="text-[6px] text-white/50">online</div>
              </div>
            </div>
            <div className="flex justify-start">
              <div className="bg-white/10 rounded-lg p-1.5 max-w-[70%]">
                Oi! Tudo bem?
              </div>
            </div>
            <div className="flex justify-end">
              <div className="bg-emerald-500/40 rounded-lg p-1.5 max-w-[70%]">
                Quero agendar
              </div>
            </div>
            <div className="flex justify-start">
            ...
            <div className="flex justify-end">
              <div className="bg-emerald-500/40 rounded-lg p-1.5 max-w-[70%]">
                <span className="text-[6px] text-white/80">✓✓</span> Fechado!
              </div>
            </div>
          </div>
        )}
        ...
        {variant === 'instagram' && (
          <div className="p-2 pt-6 text-white space-y-1">
            <div className="flex items-center gap-1">
              <div className="w-6 h-6 rounded-full bg-gradient-to-br from-purple-500 via-pink-500 to-orange-400" />
              <div className="flex-1">
                <div className="text-[8px] font-bold">@studio</div>
                <div className="text-[5px] text-white/50">há 2h</div>
              </div>
              <div className="text-[10px]">⋯</div>
            </div>
            <div className="bg-gradient-to-br from-purple-500/40 to-pink-500/40 rounded h-12" />
            <div className="flex items-center gap-1 text-[10px]">
              <span>♥</span><span className="text-[7px]">18.4K</span>
              <span className="ml-2">💬</span><span className="text-[7px]">142</span>
              <span className="ml-2">📤</span>
            </div>
            <div className="bg-white/5 rounded p-1">
              <div className="text-[6px] text-white/70">@studio Mais um dia de conteúdo</div>
            </div>
          </div>
        )}

        {variant === 'calendar' && (
          <div className="p-2 pt-6 text-white">
            <div className="text-[8px] font-bold text-center mb-1">Março 2026</div>
            <div className="grid grid-cols-7 gap-0.5 text-[6px] text-center">
              {Array.from({ length: 31 }).map((_, i) => (
                <div
                  key={i}
                  className={`aspect-square rounded-sm flex items-center justify-center ${[5, 12, 19, 26].includes(i + 1) ? 'bg-emerald-500 text-black font-bold' : 'text-white/70'}`}
                >
                  {i + 1}
                </div>
              ))}
            ...
            <div className="mt-1 space-y-0.5">
              {[5, 12, 19].map((d) => (
                <div key={d} className="bg-emerald-500/15 border-l-2 border-emerald-400 rounded-r p-0.5 text-[5px]">
                  14h Consulta
                </div>
              ))}
            </div>
...
const DashboardCard: React.FC = () => (
  <UICardContainer className="h-full">
    <div className="p-3 text-white space-y-2 h-full flex flex-col">
      <div className="flex items-center justify-between">
        <div>
          <div className="text-[7px] font-bold uppercase tracking-wider text-emerald-400">Performance</div>
          <div className="text-[10px] font-bold">Últimos 30 dias</div>
        </div>
        <div className="text-[8px] px-1.5 py-0.5 bg-emerald-500/20 text-emerald-300 rounded">↑ 186%</div>
      </div>
      <div className="grid grid-cols-2 gap-2">
        <div>
          <div className="text-[14px] font-black leading-none">452.8K</div>
          <div className="text-[6px] text-white/60 mt-0.5">Visualizações</div>
        </div>
        <div>
          <div className="text-[14px] font-black leading-none text-emerald-400">12.4K</div>
          <div className="text-[6px] text-white/60 mt-0.5">Engajamentos</div>
        </div>
      </div>
      <div className="flex-1 flex items-end gap-0.5">
        {[35, 55, 40, 65, 50, 70, 60, 80, 75, 90, 85, 95].map((h, i) => (
          <div key={i} className="flex-1 rounded-t" style={{ height: `${h}%`, background: `linear-gradient(180deg, rgba(16,185,129,0.9), rgba(16,185,129,0.5))` }} />
        ))}
      </div>
      <div className="text-[6px] text-white/50">Jan - Mar 2026</div>
    </div>
  </UICardContainer>
);
...
const CalendarCard: React.FC = () => (
  <UICardContainer className="h-full">
    <div className="p-3 text-white h-full flex flex-col">
      <div className="text-[9px] font-bold mb-1">Agendamentos</div>
      <div className="space-y-1 flex-1 overflow-hidden">
        {[
          { day: 'Seg', time: '09:00', name: 'Maria Silva', color: '#10b981' },
          { day: 'Seg', time: '14:30', name: 'João Santos', color: '#f59e0b' },
          { day: 'Ter', time: '10:00', name: 'Ana Costa', color: '#3b82f6' },
          { day: 'Qua', time: '15:00', name: 'Pedro Lima', color: '#10b981' },
        ].map((ev, i) => (
          <div key={i} className="flex items-center gap-1 bg-white/8 rounded p-1 text-[7px]">
            <div className="w-1 h-6 rounded-full" style={{ background: ev.color }} />
            <div className="flex-1">
              <div className="font-bold">{ev.name}</div>
              <div className="text-white/50">{ev.day} {ev.time}</div>
            </div>
          </div>
        ))}
      </div>
...
const ChartCard: React.FC = () => (
  <UICardContainer className="h-full">
    <div className="p-3 text-white space-y-2 h-full flex flex-col">
      <div className="flex items-center justify-between">
        <div className="text-[8px] font-bold uppercase tracking-wider text-emerald-400">Crescimento</div>
        <div className="text-[14px] font-black text-emerald-400">+186%</div>
      </div>
      <svg viewBox="0 0 100 50" className="w-full h-12">
        ...
        <path d="M0,40 L20,35 L40,28 L60,20 L80,12 L100,5 L100,50 L0,50 Z" fill="url(#chartGrad)" />
        <path d="M0,40 L20,35 L40,28 L60,20 L80,12 L100,5" stroke="#10b981" strokeWidth="2" fill="none" />
      </svg>
...
const ChatCard: React.FC = () => (
  <UICardContainer className="h-full">
    <div className="p-3 text-white space-y-1.5 h-full flex flex-col">
      <div className="flex items-center gap-1.5 pb-1 border-b border-white/10">
        ...
        <div>
          <div className="text-[9px] font-bold">Dr. Equipe</div>
          <div className="text-[6px] text-emerald-400">● online</div>
        </div>
      </div>
      <div className="space-y-1 flex-1 text-[8px]">
        <div className="flex justify-end">
          <div className="bg-emerald-500/40 rounded-2xl rounded-tr-sm px-2 py-1 max-w-[75%]">
            Oi! Tem consulta amanhã?
          </div>
        </div>
        <div className="flex justify-start">
          <div className="bg-white/10 rounded-2xl rounded-tl-sm px-2 py-1 max-w-[75%]">
            Sim, às 14h confirma?
          </div>
        </div>
        <div className="flex justify-end">
          <div className="bg-emerald-500/40 rounded-2xl rounded-tr-sm px-2 py-1 max-w-[75%]">
            Feito! <span className="text-[6px] text-white/60">✓✓</span>
          </div>
        </div>
      </div>
    </div>
...
const StarsCard: React.FC = () => (
  <UICardContainer className="h-full">
    <div className="p-3 text-white space-y-1.5 h-full flex flex-col items-center justify-center">
      <div className="text-[8px] font-bold uppercase tracking-wider text-amber-300">Pacientes Satisfeitos</div>
      <div className="flex gap-0.5 text-amber-400 text-[16px]">
        ★★★★★
      </div>
      <div className="text-[18px] font-black text-white">4.9</div>
      <div className="text-[7px] text-white/60">1.247 avaliações</div>
      <div className="flex -space-x-1 mt-1">
        {['#fbbf24', '#10b981', '#3b82f6', '#ec4899', '#8b5cf6'].map((c, i) => (
...
const renderOverlayCard = (type: string, variant?: string) => {
  switch (type) {
    case 'phone':     return <PhoneCard variant={variant || 'analytics'} />;
    case 'dashboard': return <DashboardCard />;
    case 'calendar':  return <CalendarCard />;
    case 'chart':     return <ChartCard />;
    case 'chat':      return <ChatCard />;
    case 'stars':     return <StarsCard />;
    default:          return null;
  }
};

// Templates de composicao pre-prontos
const COMPOSITION_TEMPLATES: Record<string, {
  name: string;
  emoji: string;
  description: string;
  cards: Array<{ type: string; position: { x: number; y: number }; size: { w: number; h: number }; rotation?: number; variant?: string }>;
}> = {
  'editorial-right': {
    name: 'Editorial Direita',
    emoji: '📰',
    description: 'Mockup à direita, texto à esquerda',
    cards: [
      { type: 'phone', variant: 'analytics', position: { x: 60, y: 22 }, size: { w: 32, h: 56 }, rotation: -6 },
    ],
  },
  'spotlight': {
    name: 'Spotlight Central',
    emoji: '✨',
    description: 'Mockup centralizado no topo, texto embaixo',
    cards: [
      { type: 'phone', variant: 'instagram', position: { x: 36, y: 8 }, size: { w: 28, h: 50 }, rotation: 0 },
    ],
  },
  'magazine': {
    name: 'Magazine',
    emoji: '📖',
    description: 'Editorial revista, 2 elementos distribuídos',
    cards: [
      { type: 'chart', position: { x: 56, y: 12 }, size: { w: 38, h: 32 }, rotation: -3 },
      { type: 'calendar', position: { x: 58, y: 60 }, size: { w: 36, h: 32 }, rotation: 2 },
    ],
  },
  'bento': {
    name: 'Bento Grid',
    emoji: '⬛',
    description: 'Grid estilo Apple de cards empilhados',
    cards: [
      { type: 'phone', variant: 'analytics', position: { x: 54, y: 12 }, size: { w: 20, h: 38 }, rotation: -4 },
      { type: 'dashboard', position: { x: 76, y: 12 }, size: { w: 18, h: 18 }, rotation: 2 },
      { type: 'chat', position: { x: 76, y: 33 }, size: { w: 18, h: 18 }, rotation: -2 },
      { type: 'stars', position: { x: 54, y: 56 }, size: { w: 40, h: 32 }, rotation: 1 },
    ],
  },
  'floating-cards': {
    name: 'Cards Flutuantes',
    emoji: '✨',
    description: '4 cards orbitando ao redor do texto',
    cards: [
      { type: 'phone', variant: 'chat', position: { x: 56, y: 8 }, size: { w: 20, h: 36 }, rotation: -8 },
      { type: 'chart', position: { x: 78, y: 14 }, size: { w: 18, h: 22 }, rotation: 6 },
      { type: 'calendar', position: { x: 60, y: 56 }, size: { w: 22, h: 30 }, rotation: 4 },
      { type: 'chat', position: { x: 84, y: 60 }, size: { w: 12, h: 24 }, rotation: -4 },
    ],
  },
};

function suggestOverlayFromPrompt(prompt: string): string {
  const p = prompt.toLowerCase();
  if (p.includes('smartphone') || p.includes('phone') || p.includes('tela') || p.includes('celular')) {
    if (p.includes('dashboard') || p.includes('analytics') || p.includes('métrica') || p.includes('chart')) return 'bento';
    if (p.includes('chat') || p.includes('whatsapp') || p.includes('mensagem')) return 'floating-cards';
    if (p.includes('calend') || p.includes('agenda')) return 'magazine';
    return 'editorial-right';
  }
  if (p.includes('dashboard') || p.includes('analytics') || p.includes('métrica')) return 'magazine';
  if (p.includes('calendário') || p.includes('agenda')) return 'magazine';
  if (p.includes('chat') || p.includes('conversa')) return 'floating-cards';
  if (p.includes('estrela') || p.includes('avaliação')) return 'bento';
  return 'editorial-right';
}
...
export const FlowCanvas: React.FC<FlowCanvasProps> = ({ apiKey, provider, brand }) => {
  ...
  // Template de composicao (cards UI flutuantes)
  const [overlayTemplate, setOverlayTemplate] = useState<string>(() => {
    if (typeof window === 'undefined') return 'editorial-right';
    try {
      return localStorage.getItem('flow_overlay_template') || 'editorial-right';
    } catch { return 'editorial-right'; }
  });

  useEffect(() => {
    try { localStorage.setItem('flow_overlay_template', overlayTemplate); } catch {}
  }, [overlayTemplate]);
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
          <div className="relative w-full max-w-5xl max-h-[90vh] overflow-y-auto bg-[#0d0f17] border border-white/10 rounded-3xl p-6">
            ...
            <div className="mb-5 p-4 rounded-2xl bg-white/[0.02] border border-white/10 space-y-3">
              ...
              {/* COMPOSICAO OVERLAY: Templates de cards UI flutuantes */}
              <div className="rounded-xl bg-black/20 border border-white/10 p-2.5 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-bold uppercase tracking-wider text-purple-300">
                    🎨 Cards UI sobrepostos (Mockups Premium)
                  </span>
                </div>
                <div className="grid grid-cols-5 gap-1.5">
                  {Object.entries(COMPOSITION_TEMPLATES).map(([key, tpl]) => {
                    const active = overlayTemplate === key;
                    return (
                      <button
                        key={key}
                        type="button"
                        onClick={() => setOverlayTemplate(key)}
                        ...
                        title={tpl.description}
                      >
                        <div className="text-base leading-none mb-0.5">{tpl.emoji}</div>
                        <div className="text-[7px] font-bold uppercase leading-tight">{tpl.name}</div>
                      </button>
                    );
                  })}
                </div>
                <p className="text-[9px] text-gray-400 leading-tight">
                  {COMPOSITION_TEMPLATES[overlayTemplate]?.description || 'Selecione um template'}
                </p>
              </div>
                      ...
                      {imageUrl ? (
                        <>
                          <div id={`flow-creative-preview-${i}`} className="relative aspect-square overflow-hidden bg-black">
                            ...
                            {/* UI Cards flutuantes (overlay premium) */}
                            {(COMPOSITION_TEMPLATES[overlayTemplate]?.cards || []).map((card, idx) => (
                              <div
                                key={idx}
                                className="absolute"
                                style={{
                              ...
                              >
                                {renderOverlayCard(card.type, card.variant)}
                              </div>
                            ))}
                            {logoUrlForModal && (
                              <img
